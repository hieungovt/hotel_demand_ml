...
# Configuration
st.set_page_config(
    page_title="Hotel Booking Predictions",
    page_icon="🏨",
    layout="wide"
)
...
page = st.sidebar.radio(
    "Select Feature",
    ["Cancellation Prediction", "Demand Forecast", "API Health"]
)
...
def check_api_health():
    try:
        ...
        return {"status": "error", "message": "API timeout - the free tier may be sleeping. Wait 1-2 minutes and try again."}
...
def predict_cancellation(booking_data):
    try:
        response = requests.post(
            f"{API_URL}/predict/cancellation",
            json=booking_data,
            timeout=30
        )
        response.raise_for_status()
        ...
        return {"error": "API timeout - the free tier may be sleeping. Wait 1-2 minutes and try again."}
    except requests.exceptions.ConnectionError:
...
def get_demand_forecast(days):
    try:
        response = requests.get(
            f"{API_URL}/predict/demand?days={days}",
            timeout=60
        )
        response.raise_for_status()
        return response.json()
...
if page == "Cancellation Prediction":
    ...
    with st.form("booking_form"):
        ...
        with col2:
            ...
            hotel = st.selectbox("Hotel Type", [0, 1], format_func=lambda x: "Resort" if x==0 else "City")
            deposit_type = st.selectbox("Deposit", [0, 1, 2], format_func=lambda x: ["No Deposit", "Non Refund", "Refundable"][x])
        ...
        with col3:
            ...
            market_segment = st.selectbox("Market Segment", list(range(7)), format_func=lambda x: ["Direct", "Corporate", "Online TA", "Offline TA", "Groups", "Complementary", "Aviation"][x])
            distribution_channel = st.selectbox("Distribution", list(range(5)), format_func=lambda x: ["Direct", "Corporate", "TA/TO", "GDS", "Other"][x])
            customer_type = st.selectbox("Customer Type", list(range(4)), format_func=lambda x: ["Transient", "Contract", "Transient-Party", "Group"][x])
            meal = st.selectbox("Meal Plan", list(range(5)), format_func=lambda x: ["BB", "HB", "FB", "SC", "None"][x])
    ...
    if submitted:
        ...
        if "error" in result:
            ...
            with col3:
                ...
                st.metric("Model Confidence", f"{conf_colors.get(result['confidence'], '')} {result['confidence'].title()}")
            ...
            with col1:
                ...
                risk_factors = []
                if lead_time > 100:
                    risk_factors.append(("⚠️ Long lead time", f"{lead_time} days - higher cancel risk"))
                if prev_cancellations > 0:
                    risk_factors.append(("⚠️ Previous cancellations", f"{prev_cancellations} past cancellations"))
                if deposit_type == 0:
                    risk_factors.append(("⚠️ No deposit", "No financial commitment"))
                if market_segment == 2:
                    risk_factors.append(("⚠️ Online TA booking", "OTA bookings cancel more often"))
                if special_requests == 0:
                    risk_factors.append(("⚠️ No special requests", "Less committed guest"))
            ...
            with col2:
                ...
                positive_factors = []
                if is_repeated:
                    positive_factors.append(("✅ Repeat guest", "Loyal customers rarely cancel"))
                if special_requests > 0:
                    positive_factors.append(("✅ Has special requests", f"{special_requests} requests - committed guest"))
                if deposit_type == 1:
                    positive_factors.append(("✅ Non-refundable deposit", "Financial commitment made"))
                if prev_bookings > 0:
                    positive_factors.append(("✅ Previous bookings kept", f"{prev_bookings} successful stays"))
                if lead_time < 30:
                    positive_factors.append(("✅ Short lead time", f"{lead_time} days - less time to cancel"))
    ...
    if st.button("📈 Generate Forecast", type="primary", use_container_width=True):
        ...
        if "error" in result:
            ...
            with col1:
                st.metric("Average Daily", f"{avg_daily:.1f}", help="Average predicted bookings per day")
            with col2:
                st.metric("Total Bookings", f"{total:.0f}", help="Total predicted bookings in forecast period")
            with col3:
                st.metric("Peak Day", f"{peak:.1f}", help="Highest predicted daily bookings")
            with col4:
                st.metric("Lowest Day", f"{low:.1f}", help="Lowest predicted daily bookings")
            
            # Main forecast chart
            st.markdown("---")
            st.subheader("📈 Demand Forecast with Confidence Interval")
            
            chart_df = df.set_index('date')[['predicted_bookings', 'lower_bound', 'upper_bound']]
            ...
            with col1:
                ...
                weekly_avg = df.groupby('day_name')['predicted_bookings'].mean().reindex(day_order)
                st.bar_chart(weekly_avg)
                
                # Find best/worst days
                best_day = weekly_avg.idxmax()
                worst_day = weekly_avg.idxmin()
                st.markdown(f"**Best day:** {best_day} ({weekly_avg[best_day]:.1f} avg)")
                st.markdown(f"**Slowest day:** {worst_day} ({weekly_avg[worst_day]:.1f} avg)")
            ...
            with col1:
                ...
                peak_days = df.nlargest(5, 'predicted_bookings')[['date', 'predicted_bookings']]
            ...
            with col2:
                ...
                low_days = df.nsmallest(5, 'predicted_bookings')[['date', 'predicted_bookings']]
